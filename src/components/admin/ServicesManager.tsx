...
export default function ServicesManager() {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Remove this service from the practice areas?")) return;
    ...
    <div className="mt-16">
      ...
      {editingId && (
        ...
        >
          <div className="text-[10px] font-mono uppercase tracking-[0.3em] text-[var(--color-saffron)] mb-6 flex items-center gap-3">
            <span>{editingId === "new" ? "New service" : "Edit service"}</span>
      ...
      {loading ? (
        ...
        <div className="divide-y divide-white/10 border-t border-white/10">
          {services.map((s) => {
            const hasArabic =
              !!s.title_ar?.trim() || !!s.description_ar?.trim();
            return (
              <div
                key={s.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-[oklch(0.72_0.010_75)] mt-1 truncate max-w-md">
                    {s.description}
                  </p>
                </div>
          ...
          {services.length === 0 && (
            <div className="text-[oklch(0.62_0.010_75)] text-sm py-12 text-center italic">
              No services listed yet. Add the first to begin.
            </div>
          )}
        </div>
      )}
