...
export default async function Hero() {
  ...
  const [content, images, videos] = await Promise.all([
    ...
    getVideosServer(supabase, "hero", locale),
  ]);
  ...
  const beforePhoto = images.before_photo || images.background;
  const afterVideo = videos.after_video;
      ...
      <div className="max-w-[1440px] mx-auto w-full px-5 sm:px-6 md:px-10 flex-1 grid grid-cols-12 gap-x-4 sm:gap-x-6 md:gap-x-10 pb-10 md:pb-16">
        <div className="col-span-12 lg:col-span-7 flex flex-col justify-end pb-6 lg:pb-20 pt-8 sm:pt-12 lg:pt-24">
          <div className="flex items-center gap-3 mb-6 sm:mb-10 text-[var(--color-ink-soft)]">
            ...
            <span
              className="font-mono text-[11px] tracking-[0.16em] uppercase text-[var(--color-ink-soft)]"
              style={fs("subtitle")}
            >
          ...
          <TypeWriter
            ...
            lines={[
              { text: content.title_line1 || "Architecture", style: fs("title_line1") },
              { text: content.title_line2 || "of consequence", className: "italic font-light", style: fs("title_line2") },
            ]}
          ...
          <p
            className="mt-6 sm:mt-10 max-w-[46ch] text-[var(--color-ink-soft)] leading-[1.7] text-base sm:text-[17px]"
            style={fs("lead")}
          >
            {content.lead ||
        ...
        <div className="col-span-12 lg:col-span-5 relative flex flex-col justify-end pb-6 lg:pb-20 pt-0 lg:pt-24">
          <div className="relative aspect-[3/4] sm:aspect-[4/5] w-full overflow-hidden">
            <HeroSlider
              beforePhotoUrl={beforePhoto?.url || "/hero-bg.jpg"}
              beforePhotoAlt={beforePhoto?.alt || "Before — Reference"}
              afterVideoUrl={afterVideo?.url || ""}
              afterVideoAlt={afterVideo?.alt || "After — Completed project"}
            />
            <div
              aria-hidden
          ...
          <figcaption className="mt-3 sm:mt-4 font-mono text-[10px] tracking-[0.15em] uppercase text-[var(--color-muted)]">
            Fig. 01 — {locale === "ar" ? "قبل و بعد" : "Before & After"}
          </figcaption>
        </div>
      </div>
          ...
          <dl className="grid grid-cols-3 md:grid-cols-4 divide-x divide-[var(--color-hairline)]">
            {stats.map((s, i) => (
              <div key={i} className="py-5 sm:py-6 md:py-7 px-2.5 sm:px-4 md:px-6 first:pl-0">
                <dt
                  className="font-mono text-[9px] sm:text-[10px] tracking-[0.14em] sm:tracking-[0.18em] uppercase text-[var(--color-muted)] mb-1.5 sm:mb-2 line-clamp-1"
                  style={fs(`stat${i + 1}_label`)}
                >
                  0{i + 1} — {s.label}
                </dt>
                <dd
                  className="font-display text-[clamp(1.75rem,4vw,3.75rem)] leading-none text-[var(--color-ink)] tabular-nums tracking-tight"
                  style={fs(`stat${i + 1}_number`)}
                >
                  {s.n}
